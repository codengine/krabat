...
public class PathFinder {
    ...
    public void clearMatrix(int amount) {
        numRects = amount;
    ...
    public void connectPos(int p1, int p2) {
        matrix[p1][p2] = true;
        matrix[p2][p1] = true;
    }

    // Rekursive Funktion zum Zusammensetzen des Weges
    private void searchRecursive(int aktPos, Vector<Integer> vWeg) {
    ...
    public Vector<Integer> startSearch(int startPos, int endPos) {
        this.endPos = endPos;
        vBesterWeg = new Vector<>();
        searchRecursive(startPos, new Vector<>());
        return vBesterWeg;
    }
}