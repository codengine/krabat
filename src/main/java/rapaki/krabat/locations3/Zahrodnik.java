...
import rapaki.krabat.anims.Handrij;
...
public class Zahrodnik extends Mainloc 
{
    ...
    private Borderrect reZahrodnik;
	...
	if (mainFrame.invCursor == true)
		...
		if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
			...
			if (reZahrodnik.IsPointInRect (pTemp) == true)
			    {
				// Extra - Sinnloszeug
				nextActionID = 155;
				pTemp = pZahrUnten;
			...
			// Ausreden fuer hrod
			if (lookHrod.IsPointInRect (pTemp) == true)
			    {
				// Extra - Sinnloszeug
				nextActionID = 160;
				pTemp = pHrod;
			...
			// Ausreden fuer budka
			if ((budka.IsPointInRect (pTemp) == true) && (reZahrodnik.IsPointInRect (pTemp) == false))
			    {
				// Extra - Sinnloszeug
				nextActionID = 170;
				pTemp = pBudka;
			...
			if (skizze.IsPointInRect (pTemp) == true)
			    {
				// Extra - Sinnloszeug
				nextActionID = 175;
				pTemp = pSkizze;
			...
			if (pjerjo.IsPointInRect (pTemp) == true)
			    {
				// Extra - Sinnloszeug
				nextActionID = 180;
				pTemp = pPjerjo;
			...
			if (dokumenty.IsPointInRect (pTemp) == true)
			    {
				// Extra - Sinnloszeug
				nextActionID = 185;
				pTemp = pDokumenty;
	...
	if (mainFrame.invCursor == true)
		...
		Borderrect tmp = mainFrame.krabat.KrabatRect();
		if ((tmp.IsPointInRect (pTemp) == true) || (skizze.IsPointInRect (pTemp) == true) ||
		    (papier.IsPointInRect (pTemp) == true) || (reZahrodnik.IsPointInRect (pTemp) == true) ||
		    (budka.IsPointInRect (pTemp) == true) || (pjerjo.IsPointInRect (pTemp) == true) ||
		    (dokumenty.IsPointInRect (pTemp) == true) || (lookHrod.IsPointInRect (pTemp) == true))
		    {
	...
	switch (nextActionID)
		...
		if (mainFrame.sprache == 1)
		    {
	    		...
	    		if (mainFrame.Actions[542] == false)
			    {
				...
				Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00044"), 533, 532, new int[] {532}, 615);
	    			if (mainFrame.Actions[520] == false) 	Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00045"), 532, 1000, null, 618);
	    			else                                  Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00046"), 532, 1000, null, 620);
			    }
		...
		if (mainFrame.sprache == 2)
		    {
				...
				Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00056"), 533, 532, new int[] {532}, 615);
	    			if (mainFrame.Actions[520] == false) 	Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00057"), 532, 1000, null, 618);
	    			else                                  Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00058"), 532, 1000, null, 620);
			    }
		...
		if (mainFrame.sprache == 3)
		    {
				...
				Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00068"), 533, 532, new int[] {532}, 615);
	    			if (mainFrame.Actions[520] == false) 	Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00069"), 532, 1000, null, 618);
	    			else                                  Dialog.ExtendMC (Start.stringManager.getTranslation("Loc3_Zahrodnik_00070"), 532, 1000, null, 620);
			    }
	    ...
	    case 623:
		// warten
		if (tCounter == 48) mainFrame.wave.PlayFile ("sfx-dd/pisac.wav");
		if ((--tCounter) > 1) break;
		...
		if (tCounter == 5) mainFrame.krabat.nAnimation = 93;
		if ((--tCounter) > 1) break;
		nextActionID = 628;
		handrijGibt = false;
		break;
		...
		mainFrame.inventory.vInventory.addElement (new Integer (30));
