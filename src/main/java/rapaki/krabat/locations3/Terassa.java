...
import rapaki.krabat.anims.DDKowar;
...
public class Terassa extends Mainloc
{
	...
	if ((mainFrame.Actions[529] == true) && (mainFrame.Actions[701] == false)) schmiedVisible = true;
	else schmiedVisible = false;
	...
	switch (oldLocation)
		...
		if (vorWennHinten[0].PointInside (mainFrame.krabat.GetKrabatPos ()) == true) isVordergrund = true;
		else isVordergrund = false;
		break;
	...
	if (mainFrame.krabat.nAnimation != 0)
		...
		// Rumstehen oder Laufen
		    ...
		    {
			mainFrame.krabat.drawKrabat (g);
			if (mainFrame.invCursor == true) evalMouseMoveEvent (mainFrame.Mousepoint);  // wenn Krabat unter den Umzieh-Cursor laeuft, muss das Highlight automatisch kommen
		    }
	    }  
	...
	if (schnauzeSchmied == true) return;
	...
	switch (nextActionID)
		...
		if (mainFrame.Actions[702] == false) nextActionID = 730;  // Spruch nur 1x reissen
		else nextActionID = 740;
		break;
		...
		// wieder erscheinen
		mainFrame.wegGeher.SetzeGarantiertNeuenWeg (nachUmziehPoint);
		if (mainFrame.Actions[702] == false) nextActionID = 775;
		else nextActionID = 780;
		break;
