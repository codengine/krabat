...
import rapaki.krabat.anims.Bludnicki;
import rapaki.krabat.anims.Plomja;
...
public class Labyr7 extends Mainlaby 
{
  private GenericImage background, lab72;
  ...
  private static final GenericPoint Alinks  = new GenericPoint ( 10, 331);
  private static final GenericPoint Arechts = new GenericPoint (610, 318);
  private GenericPoint AusPoint = new GenericPoint (0, 0);
  
  // Konstanten - Rects
  private static final Borderrect linkerAusgang  = new Borderrect (  0, 324,  39, 433);
  private static final Borderrect rechterAusgang = new Borderrect (593, 342, 639, 454);
  private static final Borderrect lab72Rect      = new Borderrect ( 92, 276, 372, 438);
  
  // Exitlocation - Array
  private static final int[] Exitright = {/*51,*/ 52, 53, 54, 56, 58, 59, 61};
  private static final int[] Exitleft  = {/*51,*/ 52, 53, 55, 58, 59, 60, 62};

  // Konstante Points
  private static final GenericPoint Pleft  = new GenericPoint (  0, 380);
  private static final GenericPoint Pright = new GenericPoint (639, 400);

  // Nach diesem Fusspunkt richtet sich die komplette Location !!!
  private static final GenericPoint bludFeet = new GenericPoint (515, 360);

  // Initialisierung ////////////////////////////////////////////////////////

  // Instanz von dieser Location erzeugen
  public Labyr7 (Start caller,int Richtung) 
  {
    super (caller);
    System.out.println ("Laby 7");
    ...
    Pblud    = new GenericPoint (479, 380); // Krabats Punkt zum Anschauen/Benutzen des Blud
    bludFacing = 3;
    locIndex = 57;    
  	...
  	if (mainFrame.Actions[235] == true) bludVisible = true;
  ...
  private void InitLocation (int Richtung)
  {
    ...
    mainFrame.wegGeher.vBorders.addElement (new Bordertrapez (  0, 351, 166, 405));
    mainFrame.wegGeher.vBorders.addElement (new Bordertrapez (167, 366, 390, 386));
    mainFrame.wegGeher.vBorders.addElement (new Bordertrapez (391, 371, 639, 405));
    ...
    switch (Richtung)
    {
      case 0: 
    	...
    	case 9:
        // von rechts aus
    		mainFrame.krabat.SetKrabatPos (new GenericPoint (617, 400));
        mainFrame.krabat.SetFacing (9);
        Ausgang = BerechneAusgang (false, false, true, false);
        break;
      case 3: 
        // von links aus
        mainFrame.krabat.SetKrabatPos (new GenericPoint ( 24, 380));
        mainFrame.krabat.SetFacing (3);
        Ausgang = BerechneAusgang (false, false, false, true);
        break;
    }    
    // Hier richtigen Punkt fuers Geblinker eintragen
    if (Ausgang == 3)  AusPoint = Arechts;
    if (Ausgang == 9)  AusPoint = Alinks;
  
    if (mainFrame.Actions[236] == true) isBlinker = true;
  }

  // Bilder vorbereiten
  private void InitImages() 
  {
    background = getPicture ("gfx/labyrinth/laby7.gif");
    lab72      = getPicture ("gfx/labyrinth/lab7-2.gif");
  ...
  public void paintLocation (GenericDrawingContext g)
  {
    ...
    if (lab72Rect.IsPointInRect (pKrTemp) == true)
    {
      g.drawImage (lab72, 126, 199, null);
    }
  ...
  private void evalNewLocationLeft()
  {
      ...
      while (zuffi > 62) zuffi--;
      for (int i = 0; i < Exitleft.length; i++)
      {
      	if (zuffi == Exitleft[i]) exit = true;
      }
    }  		
    newloc = zuffi;
  }
  ...
  private void evalNewLocationRight()
  {
      ...
      {
      	if (zuffi == Exitright[i]) exit = true;
      }
    }
    newloc = zuffi;  		
  }				
