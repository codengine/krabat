...
import rapaki.krabat.anims.Mlynkptack;
...
public class Mlyn1 extends Mainloc 
{
	...
	if  (outputText != "")
		...
		GenericRectangle my;
		my = g.getClipBounds();
		if (setClipForText == false) g.setClip (0, 0, 644, 484);
		else g.setClip (120, 260, 135, 145);
		mainFrame.ifont.drawString (g, outputText, outputTextPos.x, outputTextPos.y, FarbenArray[TalkPerson]);
		if (allowTextClipset == true)
		    {
			if (setClipForText == false) setClipForText = true;
			g.setClip (120, 260, 135, 145);
		    }
		else
		    {	
	...
	switch (nextActionID)
		...
		if (voegelFliegen == false) nextActionID = 11;
		break;
		...
		if (walkReady == true) nextActionID = 71;
		break;
	    ...
	    case 72:
		// warten auf ich habe fertig
		if (walkReady == true) nextActionID = 73;
		break;
