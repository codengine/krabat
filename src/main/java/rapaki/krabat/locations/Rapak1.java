...
public class Rapak1 extends Mainloc 
{
	...
	if (Sonderstatus != 0)
		...
		g.setClip (160, 160, 325, 320);
	  	if (schiesser.drawKrabat (g, mainFrame.krabat.GetKrabatPos()) == false) Sonderstatus = 0;
	    }
	else
	    {
	  	// Animation??
	...
	switch (nextActionID)
		...
		if (Sonderstatus == 0) nextActionID = 163;
		break;
		...
		nextActionID = 164;
		animit = true;
		if (mainFrame.Actions[210] == false) segeln = true;
		break;
      	
	    case 164:
		// Auf Ende beider Anims warten und dann beenden
		if ((animit == true) || (segeln == true)) break;
		mainFrame.Clipset = false;
		mainFrame.fPlayAnim = false;
		evalMouseMoveEvent (mainFrame.Mousepoint);
		mainFrame.Actions[210] = true;
