...
public class Kupa1 extends Mainloc 
{
    ...
    private GenericPoint CorrectY (GenericPoint dst)
    {
  	...
  	if (dst.x > 411) return dst;
  	if (dst.x < 262) return dst;
  	else return (new GenericPoint (dst.x, Carray [dst.x - 262] + 2));
    }		
    ...
    private GenericPoint CorrectSetY (GenericPoint dst)
    {
  	...
  	if (dst.x > 499) return dst;
  	if (dst.x > 445) return (new GenericPoint (dst.x, 350));
  	if (dst.x > 424) return (new GenericPoint (dst.x, 352));
  	if (dst.x > 411) return (new GenericPoint (dst.x, 359));

	// Hier Korrektur, wenn Schild noch nicht in Location
	if (mainFrame.Actions[224] == false)
	    {
		if (dst.x < 362) return (CorrectY (new GenericPoint (362, 300)));
	    }   
  	if (dst.x < 262) return dst; //(new GenericPoint (262, 363));
