...
import rapaki.krabat.anims.RapakWuswobodzic;
...
public class Swoboda extends Mainloc
{
	...
	Borderrect temp;
	if (mlynkHatStock == false) temp = mueller.MlynkRect();
	...
	if (rapakiAchtung == false)
		...
		for (int i = 1; i <= 12; i++)
		    {
			if ((i == KRABATRABE) && (rabeVisible == false)) continue;
			g.setClip (rapakpos[i].x, rapakpos[i].y, RapakWuswobodzic.Breite, RapakWuswobodzic.Hoehe);
		...
		for (int i = 1; i <= 12; i++)
			...
			if ((i == KRABATRABE) && (rabeVisible == false)) continue;
			g.setClip (rapakpos[i].x, rapakpos[i].y, RapakWuswobodzic.Breite, RapakWuswobodzic.Hoehe);
			if (i != KRABATRABE) raben[i].KratzeLinks (g, rapakpos[i]);
			else raben[i].KratzeRechts (g, rapakpos[i]);
		    }
	    }		  
	...
	// Clipping - Rectangle feststellen und setzen
	Borderrect tmp;
	if (mlynkHatStock == false) tmp = mueller.MlynkRect();
	else tmp = mueller.MlynkRectMitStock();
	...
	if ((TalkPerson == 36) && (mainFrame.talkCount > 0))
	    {
		if (mlynkHatStock == false) mueller.talkMlynk (g);	
		else mueller.talkMlynkWithKij (g);
	    }
      
	// nur rumstehen oder laufen
	else 
	    {
		if (mlynkHatStock == false) mueller.drawMlynk (g);
		else mueller.drawMlynkWithKij (g);
	    }  
