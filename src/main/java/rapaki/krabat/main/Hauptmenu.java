...
public class Hauptmenu extends Mainanim 
{
	...
	if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
		...
		if (brRec.IsPointInRect (pTemp) == true)
			...
			if (mainFrame.sprache == 4) mainFrame.sprache = 1; 
			InitRec ();
	...
	menuitem = 0;
	if (brNowostart.IsPointInRect  (pTemp) == true) menuitem = 1;
	if ((brWocinic.IsPointInRect   (pTemp) == true) && 
			(mainFrame.storageManager.isLoadSaveSupported() == true)) menuitem = 2;

	// Speichern nicht im Introscreen!!
	if ((brSkladzic.IsPointInRect (pTemp) == true) && (introcall == false) && 
			(mainFrame.storageManager.isLoadSaveSupported() == true)) menuitem = 3;
	if (brRec.IsPointInRect     (pTemp) == true) menuitem = 4;
	// if (brNieders.IsPointInRect   (pTemp) == true) menuitem = 5;
	if (brInfo.IsPointInRect      (pTemp) == true) menuitem = 6;
	if (brDalehrac.IsPointInRect  (pTemp) == true) menuitem = 7;
	if (brKonc.IsPointInRect      (pTemp) == true) menuitem = 8;
