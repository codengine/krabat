...
public class Inventar extends Mainanim 
{
    ...
    private GenericImage[] InventarStuecke;
    ...
    private GenericPoint pLO;                              // Offset der Linken oberen Ecke
    private Borderrect brGesamt, brMenu;            // Rectangles fuer mousemove  
    ...
    private GenericColor inakt = new GenericColor(156, 132, 107); // Farbe zum loeschen roter Rahmen
	...
	vInventory.addElement (new Integer (1)); // hier wird Floete addiert
    }           	
	...
	if (mainFrame.Clipset == false)
		...
		// Menuitems zeichnen
		if (secScreenActive == false) g.drawImage
		...
		if (nAnzahl > 9)
		    {
			secScreenAvail = true;
			if (secScreenActive == false) g.drawImage(DPfeilr, 279 + mainFrame.scrollx, 348 + mainFrame.scrolly, null);
		    }  
		else secScreenAvail = false;
		int f = 0;
		if (secScreenActive == true) f = 9;
		else if (nAnzahl > 9) nAnzahl = 9;
		...
		for (int i = f; i < nAnzahl; i++)
			...
			if ((mainFrame.whatItem != iTemp) || (mainFrame.invCursor == false))
	...
	if (nFeldAktiv >= 0) oFeldAktiv = nFeldAktiv;

	// Menuitem abdunkeln, wenn Maus weg
	switch (olditem)
	    {
	    case 0: break;
	    case 1: g.drawImage (inactiveMenu, pLO.x + 415 + mainFrame.scrollx, pLO.y + 331 + mainFrame.scrolly, null);
		break;
	    case 2: g.drawImage (DPfeill, 119 + mainFrame.scrollx, 349 + mainFrame.scrolly, null);
		break;
	    case 3: g.drawImage (DPfeilr, 279 + mainFrame.scrollx, 348 + mainFrame.scrolly, null);
		break;
	    default: 
	...
	switch (menuitem)
	    {
	    case 0: break;
	    case 1: g.drawImage(activeMenu, pLO.x + 416 + mainFrame.scrollx, pLO.y + 333 + mainFrame.scrolly, observer);
		break;
	    case 2: g.drawImage(Pfeill, 121 + mainFrame.scrollx, 350 + mainFrame.scrolly, observer);
		break;
	    case 3: g.drawImage(Pfeilr, 280 + mainFrame.scrollx, 350 + mainFrame.scrolly, observer);
		break;              
	    default: 
	...
	if (mainFrame.talkCount > 0)
		...
		if (mainFrame.talkCount != 0) return;
		else
		    {
    ...
    private GenericImage GetIconImage (int nGegID)
    {
  	...
  	for (int i = 1; i <= INVENTAR_CACHE; i++)
	    {
		if (nGegID == InventarID[i]) return (InventarStuecke[i]);
	    }
  	...
  	CachePosition++;
  	if (CachePosition == 11) CachePosition = 1;
	...
	if (mainFrame.invCursor == true)
		...
		if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
			...
			if ((brPfeilr.IsPointInRect (pTemp) == true) && (secScreenActive == false) && (secScreenAvail == true))
			    {
			...
			if (x != -1)
			    {	
				int xx = ((Integer) vInventory.elementAt (x)).intValue();
          
				// Kombination Kij + Lajna = Wuda
				if (evalKombination (2, 6, 7, xx) == true) return;

				// Kombination Wuda + Hocka = W + H
				if (evalKombination (5, 7, 9, xx) == true) return;
          
				// Kombination W + H + Wacki = W + H + W
				if (evalKombination (8, 9, 10, xx) == true) return;
          
				// Kombination W + H + W + Dryba = W + H + D + Wacki extra !!!!
				if (evalKombination (10, 13, 11, xx) == true)
				    {
					vInventory.addElement (new Integer ( 8));
					return;
				    }
          
				// Kombination W + H + Dryba = W + H + D
				if (evalKombination (9, 13, 11, xx) == true) return;
          
				// Kombination W + H + D + Wacki = W + H + W + Dryba extra
				if (evalKombination (11, 8, 10, xx) == true)
				    {
					vInventory.addElement (new Integer (13));
					return;
				    }
          
				// Kombination Kamusk + Rohodz = Bron + Kamuski -> Extrawurst in evalKombination
				if (evalKombination (12, 17, 18, xx) == true)	return;
          
				// Kombination Enterhaken + Seil = ultimatives Klettertool
				if (evalKombination (37, 38, 39, xx) == true) return;
				...
				// wacki + hornck bloto
				if (evalAusrede (8, 16, 110, xx, x) == true) return;

				// ryba + hornck bloto
				if (evalAusrede (14, 16, 115, xx, x) == true) return;
       	  
				// karta + kamuski
				if (evalAusrede (12, 20, 120, xx, x) == true) return;
       	  
				// deska + kamuski
				if (evalAusrede (12, 3, 125, xx, x) == true) return;
       	  
				// bron + kamuski
				if (evalAusrede (12, 18, 130, xx, x) == true) return;
       	  
				// pjerjo + ryba
				if (evalAusrede (14, 19, 135, xx, x) == true) return;
       	  
				// pjerjo + hornck bloto
				if (evalAusrede (16, 19, 140, xx, x) == true) return;
       	  
				// flejta + kamuski
				if (evalAusrede (1, 12, 145, xx, x) == true) return;
       	  
				// kij + kamuski
				if (evalAusrede (2, 12, 150, xx, x) == true) return;
       	  
				// flejta + dryba
				if (evalAusrede (1, 13, 155, xx, x) == true) return;

				// hocka + dryba
				if (evalAusrede (5, 13, 160, xx, x) == true) return;

				// wuda + dryba
				if (evalAusrede (7, 13, 165, xx, x) == true) return;
       	  
				// wacki + dryba
				if (evalAusrede (8, 13, 170, xx, x) == true) return;
       	  
				// kamuski + dryba
				if (evalAusrede (12, 13, 175, xx, x) == true) return;
       	  
				// wacki + ryba
				if (evalAusrede (8, 14, 180, xx, x) == true) return;
       	  
				// wuda hocka + ryba
				if (evalAusrede (9, 14, 185, xx, x) == true) return;
       	  
				// wuda wacki + ryba
				if (evalAusrede (10, 14, 190, xx, x) == true) return;
       	  
				// wuda dryba + ryba
				if (evalAusrede (11, 14, 195, xx, x) == true) return;

				// kij + wacki
				if (evalAusrede (2, 8, 200, xx, x) == true) return;

				// hornck + wacki
				if (evalAusrede (4, 8, 205, xx, x) == true) return;
       	  
				// hocka + wacki
				if (evalAusrede (5, 8, 210, xx, x) == true) return;

				// wuda + wacki
				if (evalAusrede (7, 8, 215, xx, x) == true) return;
       	  
				// hocka + lajna
				if (evalAusrede (5, 6, 220, xx, x) == true) return;
				...
				// list + 5 tolerow
				if (evalAusrede (30, 40, 230, xx, x) == true) return;
				...
				// hamor + metall
				if (evalAusrede (46, 48, 255, xx, x) == true) return;

                                // Teil 4

				// Syno auf Lichtschaale
				if (evalAusrede (63, 60, 270, xx, x) == true) return;

                                // Feuersteine auf Syno
                                if (evalAusrede (12, 63, 271, xx, x) == true) return;

                                // Feuersteine auf Koraktor
                                if (evalAusrede (12, 61, 272, xx, x) == true) return;
	...
	// Kursor ist normal, default - Reaktion
		...
		if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
			...
			if ((brMenu.IsPointInRect (pTemp) == true) && (secScreenActive == false))
			    {
				Deactivate();
			...
			if (x > -1) 
				...
				nextActionID = ((Integer) vInventory.elementAt (x)).intValue();
				mainFrame.repaint();
			    }              
		    }
		else
			...
			if (x > -1)
				...
				mainFrame.Cinventar = GenericToolkit.getDefaultToolkit().createCustomCursor
				    (GetCursorImage (((Integer) vInventory.elementAt (x)).intValue()),
				     HotSpot, "Inv");
				mainFrame.CHinventar = GenericToolkit.getDefaultToolkit().createCustomCursor
				    (GetCursorHighImage (((Integer) vInventory.elementAt (x)).intValue()),
				     HotSpot, "HInv");
				mainFrame.invCursor = true;
				mainFrame.whatItem = ((Integer) vInventory.elementAt (x)).intValue();

			    // ----PARSER_ENABLE----
			    
				// hier fuer den Sound eine Unterscheidung, ob bei nativem Player
				// der Soundbackground wegfallen soll oder nicht
				if ((mainFrame.whatItem == 1) || (mainFrame.whatItem == 12) || 
				    (mainFrame.whatItem == 17) || (mainFrame.whatItem == 51))
				    noBackgroundSound = true; // Floete, kamuski, Rohodz, halber Wosusk
				else noBackgroundSound = false;
	...
	if (((mainFrame.whatItem == erstesItem) && (xx == zweitesItem)) || ((mainFrame.whatItem == zweitesItem) && (xx == erstesItem)))
	    {
		vInventory.insertElementAt (new Integer (neuesItem), GetVektorPos (erstesItem, zweitesItem));
    	
		// Extrawurst fuer die Steine, werden bei Kombination mit dem Schilf nicht geloescht
		if (erstesItem  != 12) vInventory.removeElement (new Integer (erstesItem));
		if (zweitesItem != 12) vInventory.removeElement (new Integer (zweitesItem));
	...
	if (mainFrame.invCursor == true)
		...
		if (invTemp > -1)
		    {
			if ((((Integer) (vInventory.elementAt (invTemp))).intValue()) != mainFrame.whatItem)
			    {
		...
		menuitem = 0;
		if (brPfeill.IsPointInRect (pTemp) == true) menuitem = 2;
		if ((brPfeilr.IsPointInRect (pTemp) == true) &&
		    (secScreenAvail == true) && (secScreenActive == false)) menuitem = 3;
	...
	// Kursor normal, default - Reaktion
		...
		menuitem =0;
		if ((brMenu.IsPointInRect (pTemp) == true) && (secScreenActive == false)) menuitem = 1;
		if (brPfeill.IsPointInRect (pTemp) == true) menuitem = 2;
		if ((brPfeilr.IsPointInRect (pTemp) == true) &&
		    (secScreenAvail == true) && (secScreenActive == false)) menuitem = 3;
	...
	int Number = Numb;
	if (secScreenActive == true) Number = Numb - 9;
	Pleftup.x = (int) ( 95 + ((Number % 3) * 151));
	Pleftup.y = (int) ( 76 + ((Number / 3)   * 81));
	return (Pleftup);
    }  
	...
	if (secScreenActive == false)
	    {
		if (nAnzahl > 9) nAnzahl = 9;
		for (int i = 0; i < nAnzahl; i++)
		    {
			if (GetCurrentRect(i).IsPointInRect (Mp) == true)
			    giveback = i;
		    }
	    }
	else
    ...
    private int GetVektorPos (int first, int second)
    {
  	for (int i = 0; i < vInventory.size(); i++)
	    {
		if ((((Integer) vInventory.elementAt (i)).intValue() == first) || (((Integer) vInventory.elementAt(i)).intValue() == second))
		    {
			return i;
		    }
	    }
    ...
    {
	if (mainFrame.sprache == 1) outputText = HText;
	if (mainFrame.sprache == 2) outputText = DText;
	if (mainFrame.sprache == 3) outputText = NText;
	...
	switch (nextActionID)
		...
		if (zuffZahl == HAusreden.length) zuffZahl = 0;
