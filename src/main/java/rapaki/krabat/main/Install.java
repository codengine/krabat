...
public class Install extends Mainloc
{
	...
	*/
	...
	if (TestInstallFile() == true) nextActionID = 100;
	else drawMe = true;
	...
	if (SoundChosen != 0)
		...
		switch (olditem)
		    {
		    case 0: break;
		    case 1: g.drawImage(DPfeil, 121 + mainFrame.scrollx, 350 + mainFrame.scrolly, observer);
			break;
		    default: 
		...
		switch (menuitem)
		    {
		    case 0: break;
		    case 1: g.drawImage(Pfeil, 121 + mainFrame.scrollx, 350 + mainFrame.scrolly, observer);
			break;
		    default: 
			System.out.println("Falsches Menu-Item!!!");
		    }  
	...
	if (licenseAccepted == true)
		...
		// Hornjos
		col = FarbenArray[13];
		if ((selected != 1) && (RecChosen != 1)) col = FarbenArray[14];
		mainFrame.ifont.drawString (g, Texty[1], Rects[1].lo_point.x, Rects[1].lo_point.y, col);
    
		// Delnjos
		col = FarbenArray[13];
		if ((selected != 2) && (RecChosen != 2)) col = FarbenArray[14];
		...
		if (RecChosen != 0)
		    {
			if (RecChosen == 1) mainFrame.ifont.drawString (g, "Wuzwol#ce sej CD-player", 93, 170, FarbenArray[13]);
			else                mainFrame.ifont.drawString (g, "Wuzwol#yo se CD-player", 93, 170, FarbenArray[13]);
      
			// Win
			col = FarbenArray[13];
			if ((selected != 3)	&& (CdChosen != 3)) col = FarbenArray[14];
			mainFrame.ifont.drawString (g, Texty[3], Rects[3].lo_point.x, Rects[3].lo_point.y, col);
      
			// Lin
			col = FarbenArray[13];
			if ((selected != 4)	&& (CdChosen != 4)) col = FarbenArray[14];
			mainFrame.ifont.drawString (g, Texty[4], Rects[4].lo_point.x, Rects[4].lo_point.y, col);
      
			// Keiner
			col = FarbenArray[13];
			if ((selected != 5)	&& (CdChosen != 5)) col = FarbenArray[14];
			...
			if (CdChosen != 0)
			    {
				if (RecChosen == 1) mainFrame.ifont.drawString (g, "Wuzwol#ce sej sound-system", 93, 240, FarbenArray[13]);
				else                mainFrame.ifont.drawString (g, "Wuzwol#yo se sound-system", 93, 240, FarbenArray[13]);
        
				// Win
				col = FarbenArray[13];
				if ((selected != 6) && (SoundChosen != 6)) col = FarbenArray[14];
				mainFrame.ifont.drawString (g, Texty[6], Rects[6].lo_point.x, Rects[6].lo_point.y, col);
        
				// Java
				col = FarbenArray[13];
				if ((selected != 7) && (SoundChosen != 7)) col = FarbenArray[14];
				...
				if (SoundChosen != 0)
				    {
					if (RecChosen == 1) mainFrame.ifont.drawString (g, "Krabat znowa startowa#c", 93, 310, FarbenArray[13]);
					else          mainFrame.ifont.drawString (g, "Krabata wotnowotki startowa#y", 93, 310, FarbenArray[13]);
		...
		col = FarbenArray[13];
		if (LicenseChosen != 1) col = FarbenArray[14];
		...
		col = FarbenArray[13];
		if (LicenseChosen != 2) col = FarbenArray[14];
	...
	if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
		...
		if (licenseAccepted == true)
			...
			if (Rects[1].IsPointInRect (pTemp) == true) RecChosen = 1;
			if (Rects[2].IsPointInRect (pTemp) == true) RecChosen = 2;
			if ((Rects[3].IsPointInRect (pTemp) == true) && (RecChosen != 0)) CdChosen = 3;
			if ((Rects[4].IsPointInRect (pTemp) == true) && (RecChosen != 0)) CdChosen = 4;
			if ((Rects[5].IsPointInRect (pTemp) == true) && (RecChosen != 0)) CdChosen = 5;
			if ((Rects[6].IsPointInRect (pTemp) == true) && (CdChosen != 0)) SoundChosen = 6;
			if ((Rects[7].IsPointInRect (pTemp) == true) && (CdChosen != 0)) SoundChosen = 7;
			if ((brPfeil.IsPointInRect (pTemp) == true) && (SoundChosen != 0))
			    {
				Text2 = "";
				if (RecChosen == 1) Text2 += Rec1;
				if (RecChosen == 2) Text2 += Rec2;
        
				if (CdChosen == 3) Text2 += CD1;
				if (CdChosen == 4) Text2 += CD2;
				if (CdChosen == 5) Text2 += CD3;
        
				if (SoundChosen == 6) Text2 += Sound1;
				if (SoundChosen == 7) Text2 += Sound2;
	...
	for (int i = 1; i < Rects.length; i++)
	    {
		if (Rects[i].IsPointInRect (pTemp) ==	true) selected = i;
	    }
	...
	for (int i = 1; i < LizenzRects.length; i++)
	    {
		if (LizenzRects[i].IsPointInRect (pTemp) ==	true) LicenseChosen = i;
	    }
	if (LicenseChosen == 2) LicenseChosen = 1;
	if (LicenseChosen > 2) LicenseChosen = 2;
    
	menuitem =0;
	if (brPfeil.IsPointInRect (pTemp) == true) menuitem = 1;
	...
	if ((menuitem != olditem) || (selected != oldsel) || (LicenseChosen != OldLicense)) mainFrame.repaint();
    }
    ...
    private boolean TestInstallFile ()
    {
  	...
  	if (SearchStringInByte (Such10) == 0) return false;
  	if (SearchStringInByte (Such11) == 0) return false;
	...
	for (int i = 0; i < Feld.length; i++)
		...
		do
		    {
			if (which.charAt (j) !=	Feld[i + j]) exit = true;
			j++;
		    }
    ...
    private int SucheEnter (int Position)
    {
  	for	(int i = Position; i < Feld.length; i++)
	    {
  		if ((Feld[i] == 13) || (Feld[i] == 10)) return i;
	    }
  	return (0);
    }			
}	