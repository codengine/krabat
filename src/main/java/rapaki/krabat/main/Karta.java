...
public class Karta extends Mainanim 
{
	...
	if (mainFrame.Actions[305] == false)
	    {
		if (kulowRect.IsPointInRect   (pTemp) == true) newort = 1;
		if (njedzRect.IsPointInRect   (pTemp) == true) newort = 2;
		if (sunowRect.IsPointInRect   (pTemp) == true) newort = 3;
		if (ralbicyRect.IsPointInRect (pTemp) == true) newort = 4;
		if (jitkRect.IsPointInRect    (pTemp) == true) newort = 5;
		if (zdzaryRect.IsPointInRect  (pTemp) == true) newort = 6;
		if (kolmcRect.IsPointInRect   (pTemp) == true) newort = 7;
	    }
	else
	    {
	...
	if (newort != oldort) mainFrame.repaint();
    }
    ...
    private int evalCurrLocation (int Ziel)
    {
  	...
  	switch (Ziel)
	    ...
	    case 13:
		// Wjes
		if ( ((t > 13) && (t < 19)) || (t == 27) || ((t > 50) && (t < 63)) ) rueck = 14;
		else
		    {
			if ( (( t > 9) && (t < 13)) || ((t > 19) && (t < 24)) ) rueck = 10;
			else
			    {
				if ((t > 0) && (t < 8) && (t != 4)) rueck = 7;
				else
				    {
					if (t == 24) rueck = 24;
					else rueck = 9;
	    ...
	    case 16:
		// Villa
		if ((t == 11) || (t == 18) || ((t > 19) && (t < 23))) rueck = 18;
		else
		    {
			if ((t == 17) || ((t > 50) && (t < 63))) rueck = 17;
			else rueck = 15;
		    }
		break;
		...
		if (t == 20) rueck = 20;
		    ...
		    {
			if ( ((t > 14) && (t < 19)) || (t == 22) || ((t > 50) && (t < 63)) ) rueck = 22;
			else rueck = 11;
		    }
		break;
	    ...
	    case 70:
		// Cyrkej
		if (t == 85) rueck = 72; // von Wila2 aus Dubring2 vorgaukeln
		else rueck = 76;         // sonst Kulow vorgaukeln
		break;
	    ...
	    case 76:
		// Kulow
		if (t == 70) rueck = 70;  // aus Cyrkej kommend das auch vorgaukeln
		else
		    {
			if (t == 79) rueck = 79; // aus Mertens kommend dies vorgaukeln
			else rueck = 87; // sonst immer Wjes2 vorgaukeln
		    }
		break;
	    ...
	    case 83:
		// Rapak
		if (t == 93) rueck = 93; // bei Zdzary auch dies vorgaukeln
		rueck = 87; // ansonsten Wjes2 vorgaukeln
		break;
      	
	    case 85:
		// Villa
		if ((t == 70) || (t == 76) || (t == 79) || (t == 93)) rueck = 72; // Cyrkej2, Kulow2, Mertens2, Zdzary2 -> Dubring2
		else rueck = 80;  // ansonsten -> Njedz2
		break;	
      	
	    case 87:
		// Wjes
		if ((t == 70) || (t == 76) || (t == 79)) rueck = 76; // Cyrkej2, Kulow2, Mertens2 -> Kulow2
		else
		    {
			if ((t == 83) || (t == 93)) rueck = 83;  // Rapak2, Zdzary2 -> Rapak2
			else
			    {
				if (t == 85) rueck = 73; // Wila2 -> Hojnt2
				else rueck = 71;  // Rest -> Doma2
			    }
		    }
		break;
