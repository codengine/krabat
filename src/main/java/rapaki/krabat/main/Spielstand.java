...
public class Spielstand
{
	...
	if ((Pos > (mainFrame.storageManager.getFileSize() - 3)) && (Pos < (mainFrame.storageManager.getFileSize() + 3)))
		...
		for (int d = 0; d <= Pos; d++)
			...
			//}
			if ((Feld[d+1] != W) || (d < 43381)) Checksumme ^= (Feld[d] - W) % 256;
			else
			    {
				Where = d;
				break;
		...
		for (int l = 42380; l <= 43379; l++)
		    {
			if ((Feld[l] - W) == 0) Aktionen[l - 42380] = false;
			else Aktionen[l - 42380] = true;
		    }	
	...
	for (int i = 0;i <= 42360; i += 4)
  	...
  	for (int i = 42380; i <= 43379; i++)
	    {
  		if (mainFrame.Actions[i - 42380] == false) Feld[i] = (byte) W;
  		else Feld[i] = (byte) (1 + W);
	    }	
	...
	int Checksum = 0;
  	...
  	for (int e = 0; e <= 44100; e++)
		...
		//}
		if ((Feld[e] != W) || (e < 43381)) Checksum ^= (Feld[e] + W) % 256;
		else
		    {
			undwo = e;
			break;
