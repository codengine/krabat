...
{  
		  ...
		  GenericPoint pKrabatFeets)
  {
    ...
    if (quell.y1 > ziel.y2)
    {
      ...
      if (ziel.x3 > quell.x1) left = ziel.x3;
      else left = quell.x1;
      if (ziel.x4 < quell.x2) right = ziel.x4;
      else right = quell.x2;
      
      // kuerzesten Weg berechnen
      pBest.y = ziel.y2;
      if (pKrabatFeets.x < left) pBest.x = left;
      else
      {
        if (pKrabatFeets.x > right) pBest.x = right;
        else pBest.x = pKrabatFeets.x;
      }
      if ((mittenlauf == true) && (Math.abs (pKrabatFeets.y - quell.y1) > Math.abs (pKrabatFeets.x - ((left + right) / 2)))) pBest.x = (left + right) / 2;      
      return pBest;
    }
    ...
    {
      // erlaubtes Geradenstueck berechnen
      if (ziel.x1 > quell.x3) left = ziel.x1;
      else left = quell.x3;
      if (ziel.x2 < quell.x4) right = ziel.x2;
      else right = quell.x4;
      ...
      if ((mittenlauf == true) && (Math.abs (pKrabatFeets.y - quell.y2) > Math.abs (pKrabatFeets.x - ((left + right) / 2)))) pBest.x = (left + right) / 2;      
      return pBest;
    }
    ...
    {
    	// erlaubtes Geradenstueck berechnen
    	if (ziel.y1 > quell.y1) up = ziel.y1;
    	else up = quell.y1;
    	if (ziel.y2 < quell.y2) down = ziel.y2;
    	else down = quell.y2;
    	
    	// kuerzesten Weg berechnen
    	pBest.x = ziel.x1;
    	if (pKrabatFeets.y < up) pBest.y = up;
    	else
      {
    	 	if (pKrabatFeets.y > down) pBest.y = down;
    	  else pBest.y = pKrabatFeets.y;	
    	}	
      if ((mittenlauf == true) && (Math.abs (pKrabatFeets.x - quell.x2) > Math.abs (pKrabatFeets.y - ((up + down) / 2)))) pBest.y = (up + down) / 2;      
      return pBest;
    }
    ...
    if (quell.x1 > ziel.x2)
    {
      ...
      if ((mittenlauf == true) && (Math.abs (pKrabatFeets.x - quell.x1) > Math.abs (pKrabatFeets.y - ((up + down) / 2)))) pBest.y = (up + down) / 2;      
      return pBest;
    }  		  
