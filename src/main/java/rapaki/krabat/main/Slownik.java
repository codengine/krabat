...
public class Slownik extends Mainanim 
{
	...
	switch (olditem)
	    {
	    case 0: break;
	    case 1: g.drawImage(dpfeiloben, pPfeilOben.x + mainFrame.scrollx, pPfeilOben.y + mainFrame.scrolly, observer);
		break;
	    case 2: g.drawImage(dpfeilunten, pPfeilUnten.x + mainFrame.scrollx, pPfeilUnten.y + mainFrame.scrolly, observer);
		break;
	    default: 
	...
	switch (menuitem)
	    {
	    case 0: break;
	    case 1: g.drawImage(pfeiloben, pPfeilOben.x + mainFrame.scrollx, pPfeilOben.y + mainFrame.scrolly, observer);
		break;
	    case 2: g.drawImage(pfeilunten, pPfeilUnten.x + mainFrame.scrollx, pPfeilUnten.y + mainFrame.scrolly, observer);
		break;
	    default: 
	...
	if (e.getModifiers () != GenericInputEvent.BUTTON3_MASK)
		...
		if (brPfeilUnten.IsPointInRect (pTemp) == true)
			...
			Index += 10;
			mainFrame.Clipset = false;
			if (Index > (Nummer - 10)) Index = Nummer - 10;
			mainFrame.repaint();
		    }
	...
	menuitem = 0;
	if (brPfeilOben.IsPointInRect  (pTemp) == true) menuitem = 1;
	if (brPfeilUnten.IsPointInRect (pTemp) == true) menuitem = 2;
    
	int oldskip = Skip;
	if (brGesamt.IsPointInRect (pTemp) == false) Skip = -1;
	else
	    {
		Skip = 0;
	...
	if ((menuitem != olditem) || (Skip != oldskip)) mainFrame.repaint();
    }
