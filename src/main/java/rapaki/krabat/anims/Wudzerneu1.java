...
public class Wudzerneu1 extends Mainanim
{
  ...
  public synchronized void MoveTo (GenericPoint aim, boolean clearAnimPos)
  {
    ...
    if (clearAnimPos == true) anim_pos = 0;       // Animationsimage bei Neubeginn initialis.
    else anim_pos = 1;
  ...
  public void drawWudzer1 (GenericDrawingContext offGraph)
  {
    ...
    {	
	    ...
	    // links sitzen
	    if (direction_x == -1) MaleIhn (offGraph, angler_left_stand[Stand]);
	      
	    // rechts sitzen
	    if (direction_x == 1) MaleIhn (offGraph, angler_right_stand[Stand]);
	  }
	  else
	  {
	  	// hier wird gelaufen
  ...
  private int getLeftPos (int pox,int poy)
  {
    ...
    if (laeuftNicht == true) return (pox - (CWIDTH / 2));
    else return (pox - (CWIDTH / 4));
  }  
