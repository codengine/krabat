...
public class Korcmar extends Mainanim
{
  ...
  // false = weiterlaufen, true = stehengebleibt
	...
	if (horizontal == true)
		...
		anim_pos++;
		if (anim_pos == ((direction_y == 1) ? 6 : 5)) anim_pos = ((direction_y == 1) ? 2 : 1);
	...
	if (anim_pos == 0) anim_pos = ((yricht == 1) ? 2 : 1);       // Animationsimage bei Neubeginn initialis.
  ...
  // je nach Laufrichtung Krabat zeichnen
	...
	if (direction_y == 1) 
	    {
		// Zwinkern evaluieren
		if (anim_pos == 1) anim_pos = 0;
		else
		    {
			if (anim_pos == 0)
			    {
				int zuffi = (int) (Math.random () * 50);
				if (zuffi > 45) anim_pos = 1;
			    }
		    }				
