...
public class Dinglingerwalk extends Mainanim
{
  ...
  // je nach Laufrichtung Krabat zeichnen
	...
	if (isStanding == false)
	    {
		// nach links laufen
		if (direction_x  == -1) MaleIhn (offGraph, krabat_left_walk[anim_pos], false);
      
		// nach rechts laufen
		if (direction_x == 1) MaleIhn (offGraph, krabat_right_walk[anim_pos], false);
	    }
	else
	    {
    ...
    public int DoAnimation (GenericDrawingContext g, int AnimID)
    {
  	...
  	if (AnimCounter > 99) AnimCounter = 0;
  	
	// schaue Dich um
	if (AnimID == 2)
	    {
		if (AnimCounter < 15) MaleIhn (g, krabat_left_talk_body[(AnimCounter / 5) + 3], false);
		else
		    {
			AnimCounter = 99;
	...
	if (AnimID == 3)
	    {
		if (AnimCounter < 9) MaleIhn (g, krabat_skla[(AnimCounter / 3)], true);
		else
		    {
			AnimCounter = 99;
