...
public class KrabatDrasta extends Krabat
{
    ...
    private GenericImage[] krabat_left_take_unten2;
  ...
  // je nach Laufrichtung Krabat zeichnen
    ...
    public void DoAnimation (GenericDrawingContext g)
    {
                ...
                if (nAnimStep < 4)
                    {
                        ...
                        if (nAnimStep == 3) fAnimHelper = true;
                        nAnimStep++;
                    }
                else
                    {
		...
		break;
			...
			if ((nAnimStep == 6) || (nAnimStep == 18)) mainFrame.wave.PlayFile ("sfx-dd/schlag.wav");
                        nAnimStep++;
                    }
                else
                    {
	...
	// sorum hinstellen, dass er zur Seite schaut
  	if (GetFacing() == 12) mainFrame.krabat.SetFacing (9);
        if (GetFacing() == 6)  mainFrame.krabat.SetFacing (3);
	...
	if (GetFacing () == 9) left -= Offset;
	if (GetFacing () == 3) left += Offset;
	...
	// zeichnen und gut
	offGraph.drawImage (krabat_gibwino, left, up, Koerperbreite, Kopfhoehe + Koerperhoehe, null);
    }
