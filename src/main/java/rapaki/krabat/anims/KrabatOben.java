...
public class KrabatOben extends Krabat
{
  ...
  // Diese Routine bleibt unveraendert, unabhaengig davon, wie Krabat gerade aussieht
    public synchronized void Move ()
    {
        ...
        if ((isWalking == false) && (isWandering == false)) return;
            ...
            // Horizontal laufen, es gelten die Images left und right, gehen von 2 bis 11, von oben 1 bis 9
            {
                ...
                if (((walkto.x - (int) txps) * direction_x) <= 0)
                    {    	
                        ...
                        isWalking = false;
                        if ((isWandering == false) && (clearanimpos == true)) anim_pos = 0;
                    }	
            }
        ...
        if ((isWalking == false) && (isWandering == false))
            {
                ...
                SetKrabatPos (walkto);
                if (clearanimpos == true) anim_pos = 0;
            }	
        // if (anim_pos == 0) 
	//{
    ...
    public synchronized void MoveTo (GenericPoint aim)
    {
  	...
  	if (aim.x == ((int) xps) ) horiz = false;
  	else
            {
                ...
                if (angle > (45 * Math.PI / 180))  horiz = false;
                else horiz = true;
            }
  ...
  // je nach Laufrichtung Krabat zeichnen
    public void drawKrabat (GenericDrawingContext offGraph)
    {
  	...
  	// je nach Richtung Sprite auswaehlen und zeichnen
        if (horizontal == true)
            {
                // nach links laufen
                if (direction_x  == -1) MaleIhn (offGraph, krabato_left[anim_pos]);
    
      // nach rechts laufen
                if (direction_x == 1) MaleIhn (offGraph, krabato_right[anim_pos]);
            }
        else
            {
                // nach oben laufen
                if (direction_y  == -1) MaleIhn (offGraph, krabato_back[anim_pos]);
    
      // nach unten laufen
                if (direction_y == 1) MaleIhn (offGraph, krabato_front[anim_pos]);
            }
    }  
    ...
    public void DoAnimation (GenericDrawingContext g)
    {
		...
		break;
            ...
            case 4:    // An Tuer klopfen
                if (nAnimStep < 11) Klopfe (g, nAnimStep++);
                else
                    {
                        StopAnim ();
                        drawKrabat (g);
		...
		if (nAnimStep == 0) mainFrame.wave.PlayFile ("sfx/frohodz.wav"); ///////////////// Sound !!!!!!!!!!!!!!
                if (nAnimStep < rohodzWartezeit) SpieleRohodz (g, nAnimStep++);
                else
                    {
                        StopAnim ();
                        drawKrabat (g);
            ...
            case 146: // Wosusk essen (speziell)
                if (nAnimStep < 20)
                    {
			if (nAnimStep == 0) mainFrame.wave.PlayFile ("sfx-dd/wosusk.wav"); ////////////Sound!!!!!!!!!!!!!!!!!!!!!!
            ...
            case 154: // Feuer machen
                if (nAnimStep < 8)
                    {
			if (nAnimStep == 0) mainFrame.wave.PlayFile ("sfx/kamjeny.wav"); /////////////////// Sound !!!!!!!!!!!
    ...
    private void SpieleFloete (GenericDrawingContext g, int tCount) 
    {
        ...
        if (GetFacing() == 3)  MaleIhn (g, krabato_floeter[nFrame]);
        if (GetFacing() == 6)  MaleIhn (g, krabato_floetef[nFrame]);
        if (GetFacing() == 9)  MaleIhn (g, krabato_floetel[nFrame]);
        if (GetFacing() == 12) MaleIhn (g, krabato_floeteb[nFrame]);
    }
    ...
    private void SpieleRohodz (GenericDrawingContext g, int tCount) 
    {
        ...
        if (GetFacing() == 3)  MaleIhn (g, krabato_rfloeter[nFrame]);
        if (GetFacing() == 6)  MaleIhn (g, krabato_rfloetef[nFrame]);
        if (GetFacing() == 9)  MaleIhn (g, krabato_rfloetel[nFrame]);
        if (GetFacing() == 12) MaleIhn (g, krabato_rfloeteb[nFrame]);
    }
    ...
    private void IssWosusk (GenericDrawingContext g, int tCount)
    {
        ...
        if (GetFacing () == 3)  MaleIhn (g, krabato_wosuskr[tempvar]);
        if (GetFacing () == 6)  MaleIhn (g, krabato_wosuskf[tempvar]);
        if (GetFacing () == 9)  MaleIhn (g, krabato_wosuskl[tempvar]);
        if (GetFacing () == 12) MaleIhn (g, krabato_wosuskb[tempvar]);
    ...
    private void MacheFeuer (GenericDrawingContext g, int tCount)
    {
        ...
        if (GetFacing () == 3)  MaleIhn (g, krabato_feuerr[tempvar]);
        if (GetFacing () == 6)  MaleIhn (g, krabato_feuerf[tempvar]);
        if (GetFacing () == 9)  MaleIhn (g, krabato_feuerl[tempvar]);
        if (GetFacing () == 12) MaleIhn (g, krabato_feuerb[tempvar]);
    }

    // Lasse K das Buch lesen
    private void LiesBuch (GenericDrawingContext g, int tCount)
    {
        if (GetFacing () == 3)  MaleIhn (g, krabato_liesr);
        if (GetFacing () == 6)  MaleIhn (g, krabato_liesf);
        if (GetFacing () == 9)  MaleIhn (g, krabato_liesl);
        if (GetFacing () == 12) MaleIhn (g, krabato_liesb);
    }
                ...
                if ((--Verhinderbody) < 1)
                    {
                        ...
                        TalkLeft++;
                        if (TalkLeft == 4) TalkLeft = 2;
	  
                        TalkRight = (int) Math.round (Math.random() * 3);
                        TalkRight++;
                        if (TalkRight == 4) TalkRight = 2;
                    }  
                ...
                if ((--Verhindertalk) < 1)
                    {
                        ...
                        TalkMitte++;
                        if (TalkMitte == 5) TalkMitte = 1;
                    }  
	  
                Rede (offGraph, TalkLeft, TalkMitte, TalkRight);
            }
        else drawKrabat (offGraph);
    }
    ...
    public void describeKrabat (GenericDrawingContext offGraph)
    {
		...
		if ((--Verhindertalk) < 1)
			...
			describeMitte++;
			if (describeMitte == 5) describeMitte = 1;
		    }
