...
public class Dundak extends Mainanim
{
    ...
    public void drawDundak (GenericDrawingContext g, int TalkPerson, GenericPoint Posit, int noSound)
    {
      ...
      else 
      {
      	...
      	if ((--Verhinderstand) < 1)
      	{
		    ...
		    {
			Stand = 0;
			if (noSound < 1) evalSound ();
		    }
      		
      		// Extrawurst -> Zwinkern nicht so lange
      		if (Stand == 1) Verhinderstand = 2;
      		if (Stand == 3) Verhinderstand = ((int) ((Math.random () * 30) + 10));
      	}
	...
	if ((mainFrame.inventory.noBackgroundSound == true) && (mainFrame.invCursor == true)) return; // bei Problemen mit dem Soundsystem zurueckspringen
