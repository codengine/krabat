...
public class Swinjo2 extends Mainanim
{
    ...
    private static final int MAX_VERHINDERLOOK = 11;
    ...
    public Swinjo2 (Start caller, GenericPoint Posit)
    {
	super (caller);
	...
	if ((isDancing == false) && (oldDance == false))
		...
		switch (Look)
			...
			if ((--Verhinderlook) < 1)
			    {
				Verhinderlook = MAX_VERHINDERLOOK;
				if (((int) (Math.random () * 50)) > 25) Look = 2;
				else Look = 4;
			    }
			else
			    if (((int) (Math.random () * 50)) > 45) Look = 1;
			break;
		    case 1:
			...
			if ((--Verhinderlook) < 1)
			    {
				Verhinderlook = MAX_VERHINDERLOOK;
				if (((int) (Math.random () * 50)) > 25) Look = 3;
				else Look = 0;
			    }
			break;
		    case 3:
	...
	if ((isDancing == true) && (oldDance == true))
		...
		int tempDance = Dance;
		if (tempDance == 3) tempDance = 1;
		if (tempDance == 4) tempDance = 3;
