...
public class Bludnicki extends Mainanim
{
	...
	if ((TalkPerson == 35) && (mainFrame.talkCount > 1))
		...
		if ((VerhinderTalk++) >= MAX_VERHINDERTALK)
			...
			if ((zuffi < 7) && (zuffi > 0)) Kopf2 = zuffi;
		    }  
		...
		if (isTaking == false) offGraph.drawImage (Body[Rumpf]       , posit.x, posit.y + Offset, null); 	 
		else offGraph.drawImage (take, posit.x + 6, posit.y + Offset, null);
	    } 
	...
	// Blud beim Rumstehen
		...
		else offGraph.drawImage (take, posit.x - 6, posit.y + Offset, null);  // hier ist das take
	    }
    }
}    