...
public class Plokarka extends Mainanim
{
  ...
  // false = weiterlaufen, true = stehengebleibt
	...
	if (horizontal == true)
		...
		// Animationsphase weiterschalten
		anim_pos++;
		if (anim_pos == ((direction_y == 1) ? 3 : 4)) anim_pos = 0;
  ...
  // je nach Laufrichtung Krabat zeichnen
	...
	if (horizontal == true)
	    {
		// nach links laufen
		if (direction_x  == -1) MaleIhn (offGraph, (hasWaesche == true) ? krabatw_left[anim_pos] : krabat_left[anim_pos]);
      ...
      // nach rechts laufen
        ...
        // nach unten laufen
			if (direction_y == 1) MaleIhn (offGraph, (hasWaesche == true) ? krabatw_front[anim_pos] : krabat_front[anim_pos]);
		    }
		else
		    {
			// nach oben laufen
