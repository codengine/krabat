	...
	if ((TalkPerson == 22) && (mainFrame.talkCount > 1) && (TurnAnim > 49))
		...
		if ((Listenflag == true) || (TurnAnim > 1))
			...
			if (TurnAnim > 49)
				...
				if (Stand == 1) g.drawImage (kowar_walkl[2], (int) Walkx, (int) Walky, null);
				if (Stand == 2) g.drawImage (kowar_zwinker,  (int) Walkx, (int) Walky, null);
				drawVorder (g);		 		
    		  
				// Hier zurueckschalten auf Zuruecklaufen
				if (Listenflag == false) TurnAnim = 10;
			    }
			else
			    {
				...
				switch (TurnAnim)
					...
					if (Verhinderstrampeln == false)
						...
						WalklCount++;
						if (WalklCount == 5) WalklCount = 1;
					    }  
    		      
					// Auf "ich habe fertig" kontrollieren
					if ((Walkx <= TalkPoint.x) || (Walky >= TalkPoint.y)) TurnAnim++;
					...
					if (Verhinderstrampeln == false)
						...
						WalkrCount++;
						if (WalkrCount == 5) WalkrCount = 1;
					    }  
    		      
					// Auf "ich habe fertig" kontrollieren
					if ((Walkx >= WorkPoint.x) || (Walky <= WorkPoint.y)) TurnAnim++;
		...
		// arbeiten mit Pause ab und zu
			...
			switch (Work)
				...
				if ((--Verhinderarbeit) < 1)
				    {
					if (noTalkSchmied == false) evalSound ();
