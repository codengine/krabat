...
public class Hojnt extends Mainanim
{
  ...
  // false = weiterlaufen, true = stehengebleibt
	...
	if (((--Verhinderwalk) > 0) && (horizontal == true)) return false;
	...
	if (aim.x == ((int) xps) ) horiz = false;
		...
		if (angle > (30 * Math.PI / 180))  horiz = false;
		else horiz = true;
	    }
	...
	if (anim_pos == 0) anim_pos = 1;       // Animationsimage bei Neubeginn initialis.
		...
		anim_pos++;
		if ((horiz == true) && (anim_pos == 8)) anim_pos = 2;
		if ((horiz == false) && (anim_pos == 4)) anim_pos = 0;
	    }	
  ...
  // je nach Laufrichtung Krabat zeichnen
	...
	if (GetFacing() == 6) MaleIhn (offGraph, krabat_down_talk[talkHead]);
	else
	    {
		// reden nach rechts
		if (GetFacing() == 3) MaleIhn (offGraph, krabat_right_talk_head[talkHead], krabat_right_talk_body[talkBody]);
    	
    	// reden nach links
		if (GetFacing() == 9) MaleIhn (offGraph, krabat_left_talk_head[talkHead], krabat_left_talk_body[talkBody]);
	    }	
    }
	...
	if (GetFacing() == 6) MaleIhn (offGraph, krabat_down_talk[talkHead]);
	    ...
	    {
		// reden nach rechts
		if (GetFacing() == 3) MaleIhn (offGraph, krabat_right_talk_head[talkHead], krabat_right_talk_body[0]);
    	
    	// reden nach links
		if (GetFacing() == 9) MaleIhn (offGraph, krabat_left_talk_head[talkHead], krabat_left_talk_body[0]);
	    }	
    }
